import { useState, useEffect, useRef, useCallback } from "react";
  ...
  // ─── WhatsApp QR Connection Wrapper (powered by @whiskeysockets/baileys) ─
  function WaConnectionWrapper() {
    const [waStatus, setWaStatus] = useState<"disconnected"|"qr"|"connecting"|"connected">("connecting");
    ...
    useEffect(() => {
      ...
      async function poll() {
        try {
          ...
          if (r.ok && !cancelled) {
            const d = await r.json() as { status: string; qrDataUrl?: string };
    ...
    async function handleDisconnect() {
      ...
      try { await fetch("/api/whatsapp/disconnect", { method: "POST" }); }
      ...
      return (
        <div className="relative">
          ...
          <button
            ...
            title="قطع اتصال واتساب وإعادة ربط"
            className="fixed bottom-20 left-4 z-40 bg-background border rounded-full px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shadow-sm"
            style={{ direction: "rtl" }}>
      ...
      <>
        ...
        <div style={{
          ...
          <div style={{
            ...
            maxWidth:400, width:"100%", margin:"0 16px", textAlign:"center",
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:16 }}>
              ...
              <span style={{ fontWeight:700, fontSize:20, color:"#111" }}>ربط واتساب</span>
            </div>

            {waStatus === "qr" && qrDataUrl ? (
              <>
                <p style={{ fontSize:13, color:"#555", marginBottom:20, lineHeight:1.6, direction:"rtl" }}>
                  افتح واتساب على هاتفك ← <b>القائمة</b> ← <b>الأجهزة المرتبطة</b> ← <b>ربط جهاز</b>
                </p>
                <div style={{ background:"#f8f8f8", borderRadius:12, padding:12, display:"inline-block", marginBottom:16 }}>
                  <img src={qrDataUrl} alt="WhatsApp QR Code" style={{ width:240, height:240, display:"block", borderRadius:8 }} />
                </div>
                <p style={{ fontSize:12, color:"#999", direction:"rtl" }}>رمز QR يتجدد تلقائياً كل 30 ثانية</p>
              </>
            ) : (
              <div style={{ padding:"24px 0" }}>
                <div
                  className="animate-spin"
                  style={{ width:48, height:48, borderRadius:"50%", border:"3px solid #25d366", borderTopColor:"transparent", margin:"0 auto 16px" }}
                />
                <p style={{ fontWeight:600, fontSize:16, color:"#111", marginBottom:8 }}>
                  {waStatus === "connecting" ? "جاري الاتصال..." : "جاري تهيئة واتساب..."}
                </p>
                <p style={{ fontSize:13, color:"#666" }}>يرجى الانتظار، سيظهر رمز QR خلال لحظات</p>
              </div>
            )}
          </div>
        </div>
  ...
  export default WaConnectionWrapper;
