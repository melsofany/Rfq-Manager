import { useState } from "react";
  ...
  import { ArrowLeft, Send, CheckSquare, Square, Search, CheckCircle2, XCircle, MinusCircle, MessageCircle, Mail } from "lucide-react";
        ...
        <Layout>
          <div className="p-6 max-w-3xl space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className={`border rounded-lg p-3 text-center ${waFailed.length ? "bg-red-50 border-red-200" : "bg-muted/30 border-border"}`}>
                <p className={`text-2xl font-bold ${waFailed.length ? "text-red-700" : "text-muted-foreground"}`}>{waFailed.length}</p>
                <p className={`text-xs mt-0.5 ${waFailed.length ? "text-red-600" : "text-muted-foreground"}`}>واتساب فشل ✗</p>
              </div>
                ...
                <thead>
                  <tr className="border-b border-border text-right">
                    ...
                    <th className="px-4 py-2 text-muted-foreground text-xs font-medium text-center">
                      <span className="flex items-center justify-center gap-1"><MessageCircle size={12} /> واتساب</span>
                    </th>
                    <th className="px-4 py-2 text-muted-foreground text-xs font-medium text-center">
                      <span className="flex items-center justify-center gap-1"><Mail size={12} /> إيميل</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={r.supplierId} className="border-b border-border last:border-0">
                      ...
                      <td className="px-4 py-3 text-center">
                        {r.whatsapp?.status === "sent" && (
                          <span className="inline-flex items-center gap-1 text-green-600 text-xs"><CheckCircle2 size={13} /> أُرسل</span>
                        )}
                        ...
                        {r.whatsapp?.status === "no_phone" && (
                          <span className="inline-flex items-center gap-1 text-muted-foreground text-xs"><MinusCircle size={13} /> بدون رقم</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {r.email?.status === "sent" && (
                        ...
                        {r.email?.status === "no_email" && (
                          <span className="inline-flex items-center gap-1 text-muted-foreground text-xs"><MinusCircle size={13} /> بدون إيميل</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="p-6 max-w-4xl space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3 sm:self-end">
              ...
              >
                <Send size={14} />
                {sendMutation.isPending ? "جاري الإرسال..." : `إرسال لـ ${selectedIds.size} مورد${selectedIds.size !== 1 ? "ين" : ""}`}
              </Button>
            </div>
          </div>
