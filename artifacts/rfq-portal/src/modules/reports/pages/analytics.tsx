            ...
            {loading || empLoading ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between bg-card border border-border rounded-lg p-4">
                  <div>
                    <p className="text-sm font-medium text-foreground">تقرير أداء المشتريات الشامل</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      ملخص تنفيذي · حالة الطلبات · أداء الموردين · أداء الموظفين
                    </p>
                  </div>
                  <button
                    onClick={handlePrintReport}
                    className="flex items-center gap-2 px-4 py-2 bg-[#1e3a5f] text-white text-sm rounded-lg hover:bg-[#162d4a] transition-colors"
                  >
                    <Printer size={15} />
                    طباعة / تصدير PDF
                  </button>
                ...
                <div className="bg-card border border-border rounded-lg p-5">
                  <h2 className="font-semibold text-sm text-foreground mb-4 flex items-center gap-2">
                    <span className="w-1.5 h-4 bg-[#1e3a5f] rounded-full inline-block" />
                    المؤشرات الرئيسية
                  </h2>
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                    {[
                      { label: "إجمالي RFQs", value: stats?.totalRfqs ?? 0 },
                      { label: "طلبات مفتوحة", value: stats?.openRfqs ?? 0 },
                      { label: "الموردون", value: stats?.totalSuppliers ?? 0 },
                      { label: "العروض", value: stats?.totalOffers ?? 0 },
                      { label: "أوامر الشراء", value: stats?.totalPos ?? 0 },
                      { label: "إجمالي البنود", value: stats?.totalItems ?? 0 },
                    ].map((k) => (
                      <div
                        key={k.label}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                    {[
                      { label: "نسبة التسعير", value: `${stats?.pricingRate ?? 0}%`, color: "text-sky-600" },
                      { label: "تحويل RFQ → PO", value: `${stats?.rfqToPoRate ?? 0}%`, color: "text-emerald-600" },
                      { label: "البنود بـ PO", value: `${stats?.poRate ?? 0}%`, color: "text-amber-600" },
                      { label: "استجابة الموردين", value: `${stats?.responseRateThisMonth ?? 0}%`, color: "text-violet-600" },
                    ].map((k) => (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border">
                          <th className="text-right pb-2 text-muted-foreground font-medium">الحالة</th>
                          <th className="text-center pb-2 text-muted-foreground font-medium">العدد</th>
                          <th className="text-center pb-2 text-muted-foreground font-medium">النسبة</th>
                          <th className="pb-2"></th>
                        </tr>
                      </thead>
                      <tbody>
                        {statusData.map((s) => {
                          const pct = stats?.totalRfqs
                            ? Math.round((s.count / stats.totalRfqs) * 100)
                            : 0;
                          return (
                            <tr key={s.name} className="border-b border-border last:border-0">
                              ...
                              <td className="py-2.5 text-center font-mono font-semibold">{s.count}</td>
                              <td className="py-2.5 text-center text-muted-foreground">{pct}%</td>
                ...
                <div className="bg-card border border-border rounded-lg p-5 overflow-x-auto">
                  ...
                  {deepSuppliers.length > 0 ? (
                    <table className="w-full text-xs min-w-[640px]">
                      <thead>
                        <tr className="border-b border-border">
                          {["#", "المورد", "RFQs استلم", "عروض أرسل", "نسبة الاستجابة", "بنود PO", "نسبة الفوز بـ PO", "متوسط التسليم"].map(
                            (h) => (
                              <th
                                key={h}
                                className="pb-2 text-right text-muted-foreground font-medium whitespace-nowrap pr-4"
                              >
                                {h}
                              </th>
                            ),
                      ...
                      <tbody>
                        {deepSuppliers.map((s, i) => (
                          ...
                          >
                            <td className="py-2 text-muted-foreground pr-4">{i + 1}</td>
                            <td className="py-2 font-medium pr-4 whitespace-nowrap">{s.supplierName}</td>
                            <td className="py-2 text-center pr-4">{s.totalRfqsReceived}</td>
                            <td className="py-2 text-center pr-4">{s.totalOffersSubmitted}</td>
                            <td className="py-2 pr-4">
                              <span
                                className={`font-semibold ${(s.responseRate ?? 0) >= 70 ? "text-green-600" : (s.responseRate ?? 0) >= 40 ? "text-amber-600" : "text-red-500"}`}
                              >
                                {s.responseRate ?? 0}%
                              </span>
                            </td>
                            <td className="py-2 text-center pr-4">{s.totalPoItems}</td>
                            <td className="py-2 pr-4">
                              <span
                                className={`font-semibold ${(s.poWinRate ?? 0) >= 40 ? "text-green-600" : (s.poWinRate ?? 0) >= 20 ? "text-amber-600" : "text-muted-foreground"}`}
                              >
                                {s.poWinRate ?? 0}%
                              </span>
                            </td>
                            <td className="py-2 text-center pr-4">
                              {s.avgDeliveryDays != null ? `${s.avgDeliveryDays} يوم` : "—"}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <p className="text-sm text-muted-foreground text-center py-6">لا توجد بيانات موردين</p>
                  )}
                </div>

                {/* Preview: Employee performance */}
                {employees.length > 0 && (
                  <div className="bg-card border border-border rounded-lg p-5 overflow-x-auto">
                    <h2 className="font-semibold text-sm text-foreground mb-4 flex items-center gap-2">
                      <span className="w-1.5 h-4 bg-[#1e3a5f] rounded-full inline-block" />
                    ...
                    <table className="w-full text-xs min-w-[560px]">
                      <thead>
                        <tr className="border-b border-border">
                          {["#", "الموظف", "RFQs أرسل", "عروض استلم", "معدل الاستجابة", "نسبة الإسناد", "قيمة المشتريات"].map(
                            (h) => (
                              <th
                                key={h}
                                className="pb-2 text-right text-muted-foreground font-medium whitespace-nowrap pr-4"
                      ...
                      <tbody>
                        {employees.map((e, i) => (
                          ...
                          >
                            <td className="py-2 text-muted-foreground pr-4">{i + 1}</td>
                            <td className="py-2 font-medium pr-4 whitespace-nowrap">
                              {e.employee?.name ?? "—"}
                            </td>
                            <td className="py-2 text-center pr-4">{e.totalRfqsSent ?? 0}</td>
                            <td className="py-2 text-center pr-4">{e.totalOffersReceived ?? 0}</td>
                            <td className="py-2 pr-4">
                              <span
                                className={`font-semibold ${(e.responseRate ?? 0) >= 70 ? "text-green-600" : (e.responseRate ?? 0) >= 40 ? "text-amber-600" : "text-red-500"}`}
                              >
                                {e.responseRate ?? 0}%
                              </span>
                            </td>
                            ...
                            <td className="py-2 pr-4 font-mono text-xs">
                              {e.totalPurchaseValue != null
                                ? Number(e.totalPurchaseValue).toLocaleString(undefined, {
