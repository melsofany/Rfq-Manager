...
export default function NewCustomerRfqPage() {
  ...
  // When the user saves with a blank customer RFQ number, we prompt for
  ...
  const buildPayload = (rfqNoOverride?: string) => {
    ...
    const rfqNo = rfqNoOverride !== undefined ? rfqNoOverride : customerRfqNo.trim();
    return {
      customerId: customerId ?? null,
      customerName: customerName.trim(),
      customerRfqNo: rfqNo || undefined,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // If the customer RFQ number is blank, ask for confirmation before the
    // system auto-generates one.
    if (!customerRfqNo.trim()) {
  ...
  // User confirmed: proceed with auto-generation (send an empty RFQ number).
    ...
    <Layout>
      <div className="p-4 sm:p-6 max-w-4xl space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* hint when the RFQ number is left blank */}
          {customerRfqNo.trim() === "" && (
            <p className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
              <AlertCircle size={13} className="flex-shrink-0" />
              رقم طلب التسعير فارغ — سيُطلب منك تأكيد إنشائه تلقائياً عند الحفظ.
            </p>
          )}
        ...
        {/* Confirmation dialog: RFQ number left blank → auto-generate? */}
        {confirmAutoNumber && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="bg-card border border-border rounded-lg shadow-lg max-w-md w-full p-5 space-y-4">
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-2 sm:justify-end pt-1">
                <Button variant="ghost" onClick={() => setConfirmAutoNumber(false)}>
                  العودة لإدخال الرقم
                </Button>
                <Button onClick={confirmAndSave} disabled={createMutation.isPending}>
                  {createMutation.isPending ? "جارٍ الحفظ..." : "متابعة (إنشاء تلقائي)"}
                </Button>
              </div>
            </div>
          </div>
