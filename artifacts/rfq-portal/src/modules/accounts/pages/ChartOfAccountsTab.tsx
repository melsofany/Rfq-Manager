    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground text-sm">
          دليل الحسابات الهرمي وفق الترقيم المصري — الأصول (1xxx)، الخصوم (2xxx)، حقوق الملكية
          (3xxx)، الإيرادات (4xxx)، المصروفات (5xxx).
        </p>
                ...
                {accounts.map((a) => (
                  <tr key={a.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                    ...
                    <td className="px-3 py-2.5 text-xs">
                      ...
                      {a.nameEn && (
                        <span className="text-muted-foreground block text-xs">{a.nameEn}</span>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-xs">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs ${a.isActive ? "bg-emerald-100 text-emerald-700" : "bg-muted text-muted-foreground"}`}
                      >
