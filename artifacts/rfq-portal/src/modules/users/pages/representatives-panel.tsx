...
export default function RepresentativesPanel() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        editingId ? `/api/representatives/${editingId}` : "/api/representatives",
        {
  ...
  async function remove(id: number) {
    ...
    if (!response.ok) {
      setError(await readError(response, t("representatives.deleteError")));
      return;
    }
    await load();
  }
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-foreground">
            {editingId ? t("representatives.edit") : t("representatives.add")}
          </h2>
          {editingId && (
            <button
              onClick={() => {
        ...
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label>{t("representatives.name")}</Label>
            <Input
              required
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              placeholder={t("representatives.namePlaceholder")}
            />
          </div>
          <div className="space-y-1.5">
            <Label>{t("representatives.phone")}</Label>
            <Input
              required
              value={form.phone}
          ...
          <div className="sm:col-span-2 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={saving} className="gap-1.5">
              ...
              {saving
                ? t("employees.saving")
                : editingId
                  ? t("representatives.save")
                  : t("representatives.addButton")}
            </Button>
          </div>
        </form>
      </div>
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <UsersRound size={40} className="mx-auto text-muted-foreground/30 mb-3" />
            <p className="text-muted-foreground text-sm">{t("representatives.empty")}</p>
          </div>
        ) : (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-center gap-1">
                        <button
                          ...
                          className="p-1.5 hover:bg-blue-50 text-muted-foreground hover:text-blue-600"
                        >
                          <Pencil size={14} />
                        </button>
                        <button
                          ...
                          className="p-1.5 hover:bg-red-50 text-muted-foreground hover:text-red-600"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
