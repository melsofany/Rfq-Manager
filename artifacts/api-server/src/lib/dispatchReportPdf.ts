...
// Cortoba logo embedded as base64 (same logo used in rfqPdf.ts)
...
/**
 * Creates a PDFKit document for the dispatch report.
 * Does NOT call doc.end() — the caller must do that.
 */
export function createDispatchReportPdfDoc(opts: DispatchReportOptions): PDFKit.PDFDocument {
  ...
  // ── Helper: draw page header (returns y after header) ──────────────────────
  function drawHeader(yStart: number): number {
    ...
    try {
      ...
      doc.image(logoBuf, PAGE_W - MARGIN - 60, yStart + 7, { height: 58 });
    } catch {
  ...
  opts.suppliers.forEach((s, idx) => {
    // Page break check (leave room for footer + at least one row)
    ...
    const cells = [
      ...
      { text: openText,                color: s.linkOpened ? GREEN_C : "#aaaaaa", align: "center" as const },
      { text: offerText,               color: s.offerSubmitted ? GREEN_C : "#aaaaaa", align: "center" as const },
