...
function channel(s: DispatchSupplier): string {
  ...
  if (hasPhone && hasEmail) return "واتساب + إيميل";
  if (hasPhone) return "واتساب";
  if (hasEmail) return "إيميل";
  return "—";
}
...
export function generateDispatchReportPdf(opts: DispatchReportOptions): Promise<Buffer> {
  return new Promise((resolvePromise, reject) => {
    const pdfTimeout = setTimeout(() => reject(new Error("PDF generation timed out")), 20_000);
    try {
      ...
      function drawPageHeader(y0: number) {
        ...
        doc.font("Amiri").fontSize(20).fillColor("#ffffff")
          .text("تقرير إرسال طلب التسعير", MARGIN, y0 + 14, { lineBreak: false });
      ...
      const infoCells = [
        ...
        { label: "عدد الموردين",        value: String(opts.suppliers.length) },
      ];
      ...
      y += 10;
      doc.font("Amiri").fontSize(11).fillColor(BLUE)
        .text("قائمة الموردين الذين وصلهم طلب التسعير", MARGIN, y,
      ...
      const headers = ["#", "اسم المورد", "جهة الاتصال", "رقم الهاتف", "طريقة الإرسال", "فتح الرابط", "عرض سعر", "تاريخ الإرسال"];
      ...
      opts.suppliers.forEach((s, idx) => {
        if (y + ROW_H > PAGE_H - 28) {
          ...
          doc.font("Amiri").fontSize(8).fillColor(GOLD)
            .text("Cortoba Supplies — قرطبة للتوريدات   |   INFO@CORTOBA-SUPPLIES.COM",
        ...
        const methColor = meth.includes("واتساب") && meth.includes("إيميل") ? BLUE
          : meth.includes("واتساب") ? GREEN_C
          : meth.includes("إيميل") ? AMBER : "#888";
        const openText  = s.linkOpened ? `✓ (${s.openCount})` : "—";
        const offerText = s.offerSubmitted ? "✓ نعم" : "—";

        const cells = [
          { t: String(idx + 1),               c: "#666",    a: "center" as const },
          { t: s.supplierName || "—",          c: "#1a2a3a", a: "right"  as const },
          { t: s.contactPerson || "—",         c: "#555",    a: "right"  as const },
      ...
      doc.font("Amiri").fontSize(9).fillColor(BLUE)
        .text(
          `الإجمالي: ${opts.suppliers.length} مورد  |  واتساب: ${waCount}  |  إيميل: ${emailCount}  |  فتحوا الرابط: ${opened}  |  أرسلوا عرض سعر: ${submitted}`,
