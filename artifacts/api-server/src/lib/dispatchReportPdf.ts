...
export function generateDispatchReportPdf(opts: DispatchReportOptions): Promise<Buffer> {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    function drawHeader(yStart: number): number {
      ...
      doc.font("Amiri").fontSize(20).fillColor(WHITE)
        .text("Dispatch Report - RFQ", M, yStart + 12, { lineBreak: false });
      doc.font("Amiri").fontSize(10).fillColor(GOLD)
    ...
    function drawInfoBar(yStart: number): number {
      ...
      cells.forEach((c, i) => {
        const cx = M + i * cellW;
        doc.font("Amiri").fontSize(8).fillColor("#7a8fa6")
    ...
    function drawTableHeader(yStart: number): number {
      ...
      colLabels.forEach((h, i) => {
        doc.font("Amiri").fontSize(8.5).fillColor(WHITE)
          .text(h, tx + 2, yStart + 9, { width: colWidths[i] - 4, align: "center", lineBreak: false });
        tx += colWidths[i];
      });
      return yStart + ROW_H;
    }
    ...
    try {
      ...
      opts.suppliers.forEach((s, idx) => {
        ...
        const cells: { text: string; color: string; align: "center" | "left" }[] = [
          ...
          { text: s.linkOpened ? `Yes (${s.openCount})` : "No",      color: s.linkOpened ? GREEN : "#aaaaaa", align: "center" },
          { text: s.offerSubmitted ? "Yes" : "No",                   color: s.offerSubmitted ? GREEN : "#aaaaaa", align: "center" },
          { text: fmtDate(s.createdAt),                               color: "#666666",    align: "center" },
        ];

        let tx = M;
        cells.forEach((cell, i) => {
          doc.font("Amiri").fontSize(9).fillColor(cell.color)
