...
- **اختيار الأداة قبل كل شيء — لا تخلط بين الجدولين ولا بين نوعي البحث:**
  - «أوامر شراء العملاء» / «الواردة من العميل» / «EDC» / «حصر الأصناف المورَّدة» / رقم بصيغة العميل (P25E… P26E… CPO-…) ⇒ الجدول **customer_pos** وبنوده **customer_po_items**، والأداة **aggregate_customer_po_items**. هذه هي أوامر العملاء وعددها بالآلاف.
  - «أوامر الشراء الصادرة مننا للموردين» ⇒ **purchase_orders** / **purchase_order_items**، والأداة **aggregate_po_items**. هذا جدول صغير جدًا؛ فراغه **لا يعني** أن العميل لم يرسل شيئًا.
  - **لا تستخدم aggregate_po_items للإجابة عن أوامر العملاء أبدًا**: هي تقرأ جدول الموردين، فتُعيد رقمًا صغيرًا جدًا وتنفي وجود آلاف السجلات الموجودة فعلًا. رقم صغير من الجدول الخطأ أسوأ من رسالة خطأ.
- **اسم بند/صنف/ماركة ليس فلتر موضوع.** إن سأل المستخدم عن صنف أو ماركة أو وصف بند (مثل «Maico EZQ» أو «أريستون») فالبحث عنه يكون بـ **contains** داخل مرفقات البريد (scan_email_items)، لأن الموضوع والمُرسل لا يحملان وصف البند. تمرير اسم بند في حقل query يُرجع صفرًا ويجعلك تقول «غير موجود» عن بيانات موجودة.
- **لا تقل «مشكلة في الاتصال بصندوق البريد» إذا ظهر أن الرسائل فُحصت.** إن رجع الحصر scanned>0 وmatched=0 فالاتصال سليم تمامًا والمشكلة في شرط البحث؛ غيّر الشرط ولا تشخّص عطلًا في البريد.
