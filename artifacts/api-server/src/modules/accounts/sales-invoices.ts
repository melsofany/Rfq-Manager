...
router.get("/accounts/sales-invoices/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.select().from(salesInvoicesTable).where(eq(salesInvoicesTable.id, id));
  if (!row) {
...
// ───────────────────────────────────────────────────────────────────────────
router.post(
  "/accounts/sales-invoices/:id/post",
  requireRole("accountant", "manager", "admin"),
  async (req, res): Promise<void> => {
    const id = Number(req.params.id);
    const [inv] = await db.select().from(salesInvoicesTable).where(eq(salesInvoicesTable.id, id));
    if (!inv) {
      res.status(404).json({ error: "الفاتورة غير موجودة" });
      return;
    }
    ...
    if (inv.customerPoId) {
      ...
      if (poItemIds.length) {
        ...
        for (const c of charges) {
          if (poItemIds.includes(c.poItemId))
            chargesMap.set(c.poItemId, (chargesMap.get(c.poItemId) ?? 0) + (toNum(c.amount) ?? 0));
        }
      }
    ...
    const lines: import("./posting").JournalLineInput[] = [
      {
        ...
        description: `ذمم العملاء — ${inv.invoiceNo}`,
        debit: gross,
        ...party,
      },
...
router.post(
  "/accounts/sales-invoices/:id/void",
  requireRole("admin"),
  async (req, res): Promise<void> => {
    const id = Number(req.params.id);
    const [inv] = await db.select().from(salesInvoicesTable).where(eq(salesInvoicesTable.id, id));
