  ...
  return new Promise((resolvePromise, reject) => {
    try {
      ...
      doc
        .fontSize(11)
        .fillColor("#c8a84b")
        .text(rtl(opts.companyName || "قرطبة للتوريدات"), TITLE_X, 62, {
          width: TITLE_W,
          align: "center",
        });
      ...
      doc.text(rtl(`رقم الفاتورة: ${opts.invoiceNo}`), metaX, metaY, {
        width: metaW,
        align: "right",
      });
      doc.text(rtl(`التاريخ: ${invDate}`), metaX, metaY + 14, { width: metaW, align: "right" });
      if (dueDate)
        doc.text(rtl(`تاريخ الاستحقاق: ${dueDate}`), metaX, metaY + 28, {
          width: metaW,
          align: "right",
        });
      if (opts.customerPoNo)
        doc.text(rtl(`أمر شراء العميل: ${opts.customerPoNo}`), metaX, metaY + 42, {
          width: metaW,
          align: "right",
        });
      ...
      doc
        .fillColor("#555")
        .fontSize(9)
        .text(rtl("فاتورة إلى (العميل):"), M + 6, y + 5, { width: CW - 12, align: "right" });
      doc
        .fillColor(BLUE)
        .fontSize(12)
        .text(rtl(opts.customerName), M + 6, y + 14, { width: CW - 12, align: "right" });
      y += 30 + 10;
      ...
      headers.forEach((h, i) => {
        ...
        doc.text(h, colX(i) + 4, TABLE_Y + 7, {
          width: colX(i + 1) - colX(i) - 8,
          align: "center",
        });
      });
      y = TABLE_Y + 24;
      ...
      opts.items.forEach((it, idx) => {
        ...
        doc.text(String(idx + 1), colX(0) + 4, y + 5, {
          width: colX(1) - colX(0) - 8,
          align: "center",
        });
        const desc = [it.description, it.partNo ? `(${it.partNo})` : null]
          .filter(Boolean)
          .join(" ");
        doc.text(rtl(desc), colX(1) + 4, y + 5, { width: colX(2) - colX(1) - 8, align: "right" });
        doc.text(fmt(it.qty), colX(2) + 4, y + 5, {
          width: colX(3) - colX(2) - 8,
          align: "center",
        });
        doc.text(fmtMoney(Number(it.unitPrice ?? 0)), colX(3) + 4, y + 5, {
          width: colX(4) - colX(3) - 8,
          align: "center",
        });
        doc.text(fmtMoney(Number(it.total ?? 0)), colX(4) + 4, y + 5, {
          width: colX(5) - colX(4) - 8,
          align: "center",
        });
        y += 20;
      });
      ...
      totals.forEach(([label, val, strong], i) => {
        if (strong) {
          ...
          doc.text(val, totalsX + totalsW * 0.6, y + 6, {
            width: totalsW * 0.4 - 6,
            align: "center",
          });
          y += 24 + 4;
          ...
          doc
            .fillColor("#333")
            .text(val, totalsX + totalsW * 0.6, y, { width: totalsW * 0.4 - 6, align: "center" });
          y += 18;
        }
      });
      ...
      if (opts.notes) {
        ...
        doc
          .fillColor("#555")
          .fontSize(8)
          .text(rtl(opts.notes), M, y, { width: CW, align: "right" });
      }
      ...
      doc
        ...
        .fontSize(8)
        .text(
          rtl("هذه الفاتورة صادرة إلكترونياً من نظام قرطبة للتوريدات"),
          M,
          doc.page.height - 18,
