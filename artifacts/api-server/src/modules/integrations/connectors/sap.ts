...
async function s4Post(cfg: SapConfig, entity: string, body: unknown): Promise<unknown> {
  ...
  const csrfRes = await fetch(
    `${cfg.url}/sap/opu/odata4/sap/api_purchaseorder_2/srvd_a2x/sap/purchaseorder/0001/`,
    {
      method: "GET",
  ...
  const res = await fetch(
    `${cfg.url}/sap/opu/odata4/sap/api_purchaseorder_2/srvd_a2x/sap/purchaseorder/0001/${entity}`,
    {
      method: "POST",
...
export async function testConnection(
  ...
  try {
    if (cfg.variant === "sap-b1") {
      ...
      const res = await fetch(
        `${cfg.url}/sap/opu/odata4/sap/api_supplier/srvd_a2x/sap/supplier/0001/$metadata`,
        {
          headers: s4Headers(cfg),
        },
      );
...
export async function importSuppliers(cfg: SapConfig): Promise<ErpSupplier[]> {
  if (cfg.variant === "sap-b1") {
    const data = (await b1Get(
      cfg,
      "BusinessPartners?$select=CardCode,CardName,EmailAddress,Phone1,FullAddress&$filter=CardType eq 'cSupplier' and Active eq 'tYES'&$top=500",
    ...
    const data = (await s4Get(
      cfg,
      "api_supplier/srvd_a2x/sap/supplier/0001/Supplier?$top=500&$select=Supplier,SupplierFullName,EmailAddress,PhoneNumber",
    )) as { value: Record<string, unknown>[] };
    return (data.value ?? []).map((r) => ({
...
export async function importPurchaseOrders(cfg: SapConfig): Promise<ErpPurchaseOrder[]> {
  if (cfg.variant === "sap-b1") {
    const data = (await b1Get(
      cfg,
      "PurchaseOrders?$select=DocNum,CardName,DocStatus&$top=100&$orderby=DocNum desc",
    )) as { value: Record<string, unknown>[] };
    return (data.value ?? []).map((r) => ({
    ...
    const data = (await s4Get(
      cfg,
      "api_purchaseorder_2/srvd_a2x/sap/purchaseorder/0001/PurchaseOrder?$top=100&$select=PurchaseOrder,Supplier,PurchaseOrderStatus",
    )) as { value: Record<string, unknown>[] };
    return (data.value ?? []).map((r) => ({
